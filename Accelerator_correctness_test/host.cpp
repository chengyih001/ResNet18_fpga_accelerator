...
int main(int argc, char** argv) {
   ...
   // OpenCL Host Code
    ...
    d_type* all_y = load_data("parameters/validate_images.bin");
    d_type* all_input = load_data("parameters/testing_images.bin");

    d_type* tile_input = all_input;
    ...
    OCL_CHECK(err, err = queue.enqueueMigrateMemObjects({buffer_bn_weight_0, buffer_bn_weight_1_1, buffer_bn_weight_1_2, buffer_bn_weight_2_1, buffer_bn_weight_2_2, buffer_bn_weight_3_1, buffer_bn_weight_3_2, 
    ...
    for (int i=1; i <= TOTAL_N/N; i++) {

        conv(tile_input, output_conv1, weight_conv1, bias_conv1, FIL_1, 3, CONV_H, CONV_W, 3, 32, 32, FIL_1, H_1, W_1, 1, 1);
        ...
        OCL_CHECK(err, err = queue.enqueueMigrateMemObjects({buffer_output_conv1}, 0));
        OCL_CHECK(err, err = queue.finish());
    ...
    // layer 9
        ...
        tile_input += N*3*32*32;

        // Compare results
        int y_pred[N];
        float max_score = 0.0;
        for (int k=0; k < N; k++) {
            max_score = score[k*CLASS];
            y_pred[k] = 0;
            for (int j=1; j < CLASS; j++) {
                if (score[k*CLASS + j] > max_score) {
                    max_score = score[k*CLASS + j];
                    y_pred[k] = j;
                }
            }
        }
        ...
        if (i%(100/N) == 0) {
            printf("Accuracy up to iteration %d = %.2f%%\n", i*5, float(100*sum/total));
        }
    }
